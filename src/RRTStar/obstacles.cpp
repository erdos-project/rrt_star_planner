...
bool Obstacles::isSegmentInObstacle(Vector2f &p1, Vector2f &p2)
{
    ...
    for(int i = 0; i < (int)obstacles.size(); i++) {
        ...
        QLineF lseg1(obstacles[i].first.x(), obstacles[i].first.y(),
                obstacles[i].first.x() + length, obstacles[i].first.y());
        QLineF lseg2(obstacles[i].first.x(), obstacles[i].first.y(),
                obstacles[i].first.x(), obstacles[i].first.y() + breadth);
        QLineF lseg3(obstacles[i].second.x(), obstacles[i].second.y(),
                obstacles[i].second.x(), obstacles[i].second.y() - breadth);
        QLineF lseg4(obstacles[i].second.x(), obstacles[i].second.y(),
                obstacles[i].second.x() - length, obstacles[i].second.y());
        ...
        if (obstacles[i].first.x() < p1.x() &&
            p1.x() < obstacles[i].second.x() &&
            obstacles[i].first.y() < p1.y() &&
            p1.x() < obstacles[i].second.y())
            return true;
    }
    return false;
}
